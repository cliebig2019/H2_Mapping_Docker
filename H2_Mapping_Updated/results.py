...
def get_path(df, end_plant_tuple, centralised):
    """Gets the path of transport taken from the cheapest found solution."""
    min_cost = min(df['Total Cost per kg H2'])
    mindex = df.index.values[df['Total Cost per kg H2'] == min_cost]
    mindex = mindex[0]
    ...
    end_nh3_options = [nh3_costs(truck_dist=driving_distance_end, convert=False, centralised=centralised),
                       ...
                       h2_gas_costs(pipe_dist=direct_distance_end),
    ...
    start_nh3_options = [
        ...
        nh3_costs(pipe_dist=df.at[mindex, 'Gen-Port Direct Dist.'], convert=False, centralised=centralised),
        h2_gas_costs(pipe_dist=df.at[mindex, 'Gen-Port Direct Dist.']),
    ...
    total_nh3_options = [(cost_start_nh3 + cost_shipping_nh3 + cost_end_nh3),
                         ...
                         nh3_costs(pipe_dist=direct_distance_total, centralised=centralised)]
    ...
    total_h2_gas_options = [h2_gas_costs(truck_dist=driving_distance_total),
                            h2_gas_costs(pipe_dist=direct_distance_total)]
