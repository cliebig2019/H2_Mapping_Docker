...
def transport_costs(df, end_plant_tuple, h2_demand, centralised=True, pipeline=True, max_pipeline_dist=2000):
    ...
    for i in range(len(df)):
        ...
        """NH3"""
        nh3_gen_to_port_trucking=nh3_trucking_costs(truck_dist=df.at[i, 'Gen-Port Driving Dist.'], convert=False, centralised=centralised)
        nh3_gen_to_port_piping=nh3_piping_costs(pipe_dist=df.at[i, 'Gen-Port Direct Dist.'] * 1.2, convert=False, centralised=centralised, max_pipeline_dist=max_pipeline_dist)
        h2_gas_gen_to_port_trucking=h2_gas_trucking_costs(truck_dist=df.at[i, 'Gen-Port Driving Dist.'])
        h2_gas_gen_to_port_piping=h2_gas_piping_costs(pipe_dist=df.at[i, 'Gen-Port Direct Dist.'] * 1.2, max_pipeline_dist=max_pipeline_dist)

        if pipeline == True:
            start_nh3_options = [nh3_gen_to_port_piping, nh3_gen_to_port_trucking, h2_gas_gen_to_port_piping, h2_gas_gen_to_port_trucking]
        else:
        ...
        nh3_trucking_all_way=nh3_trucking_costs(truck_dist=driving_distance_total, centralised=centralised)
        nh3_piping_all_way=nh3_piping_costs(pipe_dist=direct_distance_total * 1.2, centralised=centralised, max_pipeline_dist=max_pipeline_dist)
